...
class Trainer:
    ...
    def __init__(self, opt):
        ...
        val_obj_dataset = ListDataset('data/coco/5k.txt', augment=False, multiscale=self.opt.multiscale_training)
    ...
    def run_epoch(self):
        ...
        for i, (inputs, cells) in enumerate(self.train_dataloader):
            ...
            try:
                ...
                self.obj_iter = iter(self.obj_dataloader)
                _, imgs, targets = self.obj_iter.next()
            losses = {'total': 0.,'obj_loss': 0.}
            losses['obj_loss'], predicts = self.model(imgs.to(self.device), targets.to(self.device))
            ##########################################################
            ...
            if i % self.opt.log_period == 0 and i != 0:
                ...
                self.log('train', inputs, cells, None, log_losses)
    ...
    def val(self):
        ...
        print(f"val loss:  obj: {loss_mean['obj_loss']}")
            #################################################################

        self.log('val', inputs, cells, None, loss_mean)
        self.model.train()
