...
class SALCell_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_i, cells_file,img_nr
...
class SALCell_Dataset_test(Dataset):
    def __init__(self, data_path, shape_r, shape_c):
        self.shape_r = shape_r
        self.shape_c = shape_c
        #self.batch_size = batch_size
        ...
        print(f'Dataset : {self.img_p}, number : {self.__len__()}')
    ...
    def __getitem__(self, index):
        ...
        #print(img_nr)
        img_file = os.path.join(self.img_p, img_nr)
        img_i = cv2.imread(str(img_file))
        img_i = cv2.cvtColor(img_i,cv2.COLOR_BGR2RGB)
        img_i = self.transform(img_i)
