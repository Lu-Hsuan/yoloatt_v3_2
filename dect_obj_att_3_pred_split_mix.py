...
def tar2dec(target_gt):
    #cls_pred,x1, y1, x2, y2
    # to
    #x1, y1, x2, y2, conf, cls_conf, cls_pred
    N = target_gt.size()[0]
    target_gt_ = torch.ones(N,7)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_all", help="save img", action="store_false")
    parser.add_argument("--show_all", help="save compare two", action="store_true")
    ...
    for img_i, (path,detections,map_p,std_p,map_g,std_g) in enumerate(zip(imgs,img_detections,map_p_list,std_p_list,map_g_list,std_g_list)):
        ...
        if(opt.show_all == True):
            ...
            for k,v in img_dic.items():
                ...
                idx += 1
            set_plt_img(fig,f"{save_p}/{path.split('.')[0]}_pred",img_size_r=img.shape[0]*idx,img_size_c=img.shape[1]*idx)
