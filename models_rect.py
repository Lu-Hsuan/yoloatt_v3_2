...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, img_dim=416):
        ...
        self.metrics = {}
        ###### make it supprot rectangle input #####
    ...
    def compute_grid_offsets(self, grid_size, cuda=True):
        ...
        self.stride_x = self.img_dim[1] / gx    # cell size x
        self.stride_y = self.img_dim[0] / gy    # cell size y
        # Calculate offsets for each grid (just like mesh grid)
        self.grid_x = torch.arange(gx).repeat(gy, 1).view([1, 1, gy, gx]).type(FloatTensor)
        self.grid_y = torch.arange(gy).repeat(gx, 1).t().view([1, 1, gy, gx]).type(FloatTensor)
        # Transfer anchors size scale to cell level
        self.scaled_anchors = FloatTensor([(a_w / self.stride_x, a_h / self.stride_y) for a_w, a_h in self.anchors])    
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        pred_boxes_output = pred_boxes.view(num_samples, -1, 4)
        pred_boxes_output[..., 0] *= self.stride_x
        pred_boxes_output[..., 2] *= self.stride_x
        pred_boxes_output[..., 1] *= self.stride_y
        pred_boxes_output[..., 3] *= self.stride_y
...
if __name__ == "__main__":
    ...
    #         print('fix')
    #         m.eval()
    #         for k,param in m.name_parameters().items():
    #             print(k)
