...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    best_ious, best_n = ious.max(0)     # value and index
    # Separate target values
    b, target_labels = target[:, :2].long().t() # (batch index), label
    gx, gy = gxy.t()
    gw, gh = gwh.t()
    gi, gj = gxy.long().t() # get which cell box belong to
    # Set masks
